...
void BasicTFTreeGenerator::setupTf() {
  ...
  tf2_fix_transforms_.emplace_back(
      getTransformation("map", generateTfName(ns, "odom"), 0, 0, 0, 0, 0, 0));
  ...
  RCLCPP_INFO(get_logger(), "%s", odom2base_link_tf_.header.frame_id.c_str());
  RCLCPP_INFO(get_logger(), "%s", odom2base_link_tf_.child_frame_id.c_str());
